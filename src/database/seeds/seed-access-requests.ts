...
async function main() {
  ...
  for (const p of patients) {
    ...
    if (!patLogin.token) {
      console.log(`  ✗ ${p.email} - login failed`);
      continue;
    }
    ...
    const accessReq = await req('/doctors/access-requests', {
      method: 'POST',
      token: drToken,
      body: { patientId },
    });
    ...
    if (!requestId) {
      console.log('  ✗ Access request failed:', accessReq.message || accessReq);
      continue;
    }
    console.log(`  → Request: ${requestId}`);

    // Patient approves
    const approve = await req(`/doctors/access-requests/${requestId}/respond`, {
      method: 'POST',
      token: patToken,
  ...
  // Re-login to refresh
  const dr2 = await req('/auth/login', {
    method: 'POST',
    body: { email: 'fernando.luckesi.dr@gmail.com', password: '958969' },
  });
  ...
  for (let i = 0; i < myPatients.length; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      await req(`/patients/${pid}/consultations`, {
        method: 'POST',
        token,
        body: {
          date: d.toISOString(),
    ...
    // Diseases
    if (i % 2 === 0) {
      await req(`/patients/${pid}/diseases`, {
        method: 'POST',
        token,
        body: { name: 'Hipertensão Arterial', status: 'in_treatment', diagnosisDate: '2020-05-10' },
      });
    ...
    // Allergies
    if (i % 3 === 0) {
      await req(`/patients/${pid}/allergies`, {
        method: 'POST',
        token,
        body: { name: 'Dipirona', severity: 'moderate', reaction: 'Urticária' },
      });
      console.log(`    ✓ 1 alergia`);
    }

    // Vaccines
    await req(`/patients/${pid}/vaccines`, {
      method: 'POST',
      token,
      body: {
        name: 'Influenza 2026',
